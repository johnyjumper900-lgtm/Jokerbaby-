...
const OPTION_PATTERNS: Array<{ re: RegExp; type: string; label: (m: RegExpMatchArray) => string }> =
  [
    {
      re: /\b(?:plus|\+)\s*de?\s*(\d+[.,]?\d*)\s*buts?\b/i,
      type: "Total buts",
      label: (m) => `+${m[1].replace(",", ".")} buts`,
    },
    {
      re: /\b(?:moins|-)\s*de?\s*(\d+[.,]?\d*)\s*buts?\b/i,
      type: "Total buts",
      label: (m) => `-${m[1].replace(",", ".")} buts`,
    },
    {
      re: /\bles?\s*2?\s*[ée]quipes?\s*marquent?\b|\bbtts\s*oui\b|\bbtts\b/i,
      type: "Les deux marquent",
      label: () => "Oui",
    },
    { re: /\b1x2\b.*?\b(1|x|2)\b/i, type: "1X2", label: (m) => m[1].toUpperCase() },
    {
      re: /\bvictoire\s+(.+?)(?:\s*@|\s*\d|$)/i,
      type: "1X2",
      label: (m) => `Victoire ${m[1].trim()}`,
    },
    { re: /\bmatch\s*nul\b|\bnul\b/i, type: "1X2", label: () => "Nul" },
    {
      re: /\bdouble\s*chance\s*(1x|x2|12)/i,
      type: "Double chance",
