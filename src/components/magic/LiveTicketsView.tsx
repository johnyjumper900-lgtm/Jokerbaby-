...
function judgePick(pick: Pick, h: number | null, a: number | null): LivePick["liveResult"] {
  ...
  if (
    /h2h|1x2|résultat|match winner|moneyline/.test(type) ||
    /domicile|home|extérieur|away|nul|draw|x|^1$|^2$/.test(opt)
  ) {
...
export const LiveTicketsView = ({ tickets }: LiveTicketsViewProps) => {
  const validated = useMemo(() => tickets.filter((t) => t.status === "validated"), [tickets]);
        ...
        for (const p of picks) {
          ...
          const judged =
            p.result && p.result !== "pending"
              ? p.result
              : sc.status === "finished" || sc.status === "live"
                ? judgePick(p, sc.homeScore, sc.awayScore)
                : "pending";
          arr.push({
            pick: p,
            status: sc.status,
      ...
      <HoloCard variant="violet">
        <div className="p-8 text-center">
          ...
          <p className="mt-3 text-xs text-muted-foreground">
            Aucun ticket validé.
            <br />
            Valide un ticket pour le suivre en direct ici.
          </p>
        </div>
      </HoloCard>
    );
    ...
    <div className="flex flex-col gap-3">
      ...
      {validated.map((t) => {
        ...
        const settled = live.filter(
          (l) => l.liveResult === "won" || l.liveResult === "lost",
        ).length;
        ...
        const remaining = live.filter(
          (l) => l.liveResult === "pending" || l.liveResult === "unknown",
        );
        const wonOdds = live
        ...
        const remainingProb = remaining.reduce((acc, l) => acc * (l.pick.probability / 100), 1);
        const cashOut = lostAlready
          ? 0
          : Math.max(
              0,
              (stake * wonOdds * remainingProb * totalOdds * 0.92) / Math.max(totalOdds, 1),
            );
            ...
            <div className="p-4 flex flex-col gap-3">
              ...
              <div className="space-y-2">
                {live.map((l, i) => {
                  ...
                  return (
                    <div key={i} className={`rounded-lg p-2 border ${cls}`}>
