 ...
 */
export const HoloLogo = ({ icon: Icon, size = 44, orbit = true }: HoloLogoProps) => {
  const px = `${size}px`;
  const inner = size - 6;
  ...
  return (
    ...
    >
      ...
      <motion.span
        ...
        style={{
          boxShadow: "0 0 14px oklch(from var(--primary) l c h / 0.55), 0 0 26px oklch(from var(--secondary) l c h / 0.35)",
        }}
      ...
      >
        ...
        >
          <Icon
            ...
            className="text-foreground drop-shadow-[0_0_8px_oklch(from_var(--primary)_l_c_h/0.8)]"
          />
        </motion.div>
      </div>
