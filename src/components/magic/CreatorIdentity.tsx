...
export const CreatorIdentity = () => {
  ...
  const [fp, setFp] = useState<string>(() => {
    try {
      return localStorage.getItem(VOICE_FP_KEY) || "";
    } catch {
      return "";
    }
  });
  ...
  useEffect(() => {
    ...
    saveTimer.current = window.setTimeout(() => {
      try {
        localStorage.setItem(CREATOR_KEY, name.trim());
      } catch {
        /* noop */
      }
    }, 400);
      ...
      <div className="p-4 flex flex-col gap-3">
        ...
        <div className="flex flex-col gap-2 rounded-xl border border-primary/30 bg-card/40 p-3">
          ...
          <p className="text-[10px] text-muted-foreground/80 leading-snug">
            Capture 3 secondes de timbre vocal (moyenne FFT locale, non envoyée). Stocké sur ton
            appareil.
          </p>
        </div>
      </div>
