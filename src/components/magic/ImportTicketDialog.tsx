...
function compressImageForVision(file: File): Promise<{ base64: string; mimeType: string }> {
  return new Promise((resolve, reject) => {
    const img = new Image();
...
export const ImportTicketDialog = ({ open, onClose, onImport }: ImportTicketDialogProps) => {
  ...
  const handleFile = async (file: File) => {
    ...
    try {
      ...
      if (isImage) {
        const optimized = await compressImageForVision(file);
        await runAiParse({
  ...
  const handleImport = () => {
    ...
    onImport(ticket);
    // Demande automatiquement à Magic d'analyser et de proposer son pronostic
    try {
      const summary = preview
        .map((p, i) => `${i + 1}. ${p.match} — ${p.type}: ${p.option} @${p.odds.toFixed(2)}`)
        .join("\n");
      const prompt =
        `Je viens de scanner ce ticket (mise ${stake}€${bookmaker ? `, ${bookmaker}` : ""}, cote totale ${totalOdds.toFixed(2)}):\n${summary}\n\n` +
        `Analyse-le maintenant en profondeur : pour CHAQUE sélection donne ton verdict (✅ valide / ⚠️ risqué / ❌ à remplacer) avec une raison courte. ` +
        `Puis propose TON PROPRE pronostic recomposé (mélange multichances + handicap + buts + mi-temps, jamais 100% victoires sèches) avec niveau de confiance. ` +
        `Tu dois OBLIGATOIREMENT trancher et livrer un pick actionnable, aucun refus toléré.`;
      window.dispatchEvent(new CustomEvent("magic.analyze", { detail: { prompt } }));
    } catch {
      /* noop */
    }
    setText("");
        ...
        >
          ...
          >
            ...
            <p className="text-[10px] text-muted-foreground leading-relaxed">
              Gemini lit directement ta <strong className="text-foreground">photo</strong>, ton{" "}
              <strong className="text-foreground">PDF</strong> ou le texte du ticket et extrait tous
              tes paris automatiquement.
            </p>
            ...
            <div className="grid grid-cols-2 gap-2">
              <label className="tap flex flex-col items-center justify-center gap-1 px-3 py-3 rounded-xl border-2 border-dashed border-primary/40 text-primary hover:bg-primary/5 cursor-pointer">
                <ImageIcon size={16} />
                <span className="text-[9.5px] font-bold uppercase tracking-widest">Photo</span>
                <input
                  type="file"
                  accept="image/*"
                  capture="environment"
              ...
              <div className="flex flex-col gap-1.5 bg-muted/20 rounded-xl p-2 border border-border/40">
                ...
                {preview.map((p, i) => (
                  <div key={i} className="flex items-center justify-between text-[10.5px] gap-2">
                    ...
                    <span className="font-display font-black holo-text">{p.odds.toFixed(2)}</span>
                  </div>
                ))}
