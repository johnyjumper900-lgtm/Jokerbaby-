...
export function saveUserProfile(p: UserProfile) {
  try {
    ...
    // Synchronise avec le reste de l'app (CreatorIdentity, coachContext, etc.)
    localStorage.setItem("magic.creator.name", p.firstName);
    if (p.voiceFingerprint?.length) {
      localStorage.setItem(
        "magic.voice.fingerprint",
        p.voiceFingerprint.slice(0, 16).map((n) => n.toFixed(3)).join(","),
      );
    }
  } catch {
    /* noop */
...
export const OnboardingScreen = ({ onComplete }: Props) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4 bg-aurora bg-background/80 backdrop-blur-sm">
            ...
            >
              ...
              <button
                ...
                className={`tap w-full py-4 rounded-xl flex items-center justify-center gap-2 font-display font-black uppercase tracking-[0.18em] text-[11px] ${
                  recording
                    ? "bg-destructive/80 text-primary-foreground animate-pulse"
